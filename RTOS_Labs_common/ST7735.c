...
// 
/*
static void clearMessageLine(uint16_t x, uint16_t y){
	char* clearString =   "                     ";
	ST7735_DrawString(x, y, clearString, ST7735_CYAN);
}
*/
...
#define MIDDLE_SCREEN_DEMARCATION 9
#define START_INDEX_INT_STRING 16
#define INT_STRING_MAX_LENGTH 5
// TODO: clear string and then print!

void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
	
	OS_bWait(&LCDFree);
	char* ptDupMesgLen = pt;
	...
	int16_t messageColor = ST7735_CYAN;
	uint32_t messageLengthLimit = START_INDEX_INT_STRING;
	if(messageLength > messageLengthLimit && value != -10000){
		pt[messageLengthLimit] = '\0';
		messageColor = ST7735_RED;
	}
  // write this as part of Labs 1 and 2
	uint32_t stringXCoord = 0;
	uint32_t stringYCoord = (d == 0) ? l : l + MIDDLE_SCREEN_DEMARCATION;
	
	//clearMessageLine(stringXCoord, stringYCoord); 
	ST7735_DrawString(stringXCoord, stringYCoord, pt, messageColor);
	
	
	//can change text color to denote display overflow
	//overflow into next line
	char signedIntString[INT_STRING_MAX_LENGTH];
	sprintf(signedIntString, "%d", value);
	
	uint32_t intStringLength = 0;
	char* ptDup = signedIntString;
	while(*ptDup){
		intStringLength++;
		ptDup++;
	}
	...
	if(value < -9999){
		...
		uint32_t valueXCoord = START_INDEX_INT_STRING + (INT_STRING_MAX_LENGTH - intStringLength);
		//clearMessageLine(valueXCoord, stringYCoord); 
		ST7735_DrawString(valueXCoord, stringYCoord, signedIntString , ST7735_CYAN);
	}
	
	//ST7735_DrawFastHLine(0, _height/2, _width, ST7735_RED);
	OS_bSignal(&LCDFree);
}
